    ...
    static float IC_Angle(const Mat &image, Point2f pt, const vector<int> &u_max)
    {
        ...
        for (int v = 1; v <= HALF_PATCH_SIZE; ++v)
        {
            ...
            for (int u = -d; u <= d; ++u)
            {
                ...
                m_y += u * (val_plus + val_minus);
            }
            m_x += v * v_sum;
        }

        return fastAtan2((float)m_x, (float)m_y);
    }
...
#undef GET_VALUE
    ...
    int FeatureExtractor::excute(InputArray _image, InputArray _mask, vector<KeyPoint> &_keypoints,
